...
chats = {}
allowed = []
TOKEN = "6522907112:AAGFwnr7L9qdhhS88LvsS-9alVvyJ9EwPnw"
PASSWORD = "arnetsmg"
        ...
        if txt != "":
            if "/add_tag" == txt[:8]:
                ...
                if len(txt_split) == 2 and "#" == txt_split[1][0]:
                    ...
                    bot.sendMessage(chat_id, "Format SALAH. Format seharusnya _/add_tag #{tag}_", parse_mode="Markdown")
            elif "/remove_tag" == txt[:11]:
                txt_split = txt.strip().split(" ")
                if len(txt_split) == 2 and "#" == txt_split[1][0]:
                    tag = txt_split[1].lower()
                    if tag in chats:
                    ...
                    bot.sendMessage(chat_id, "Format SALAH. Format seharusnya _/remove_tag #{tag}_", parse_mode="Markdown")
                ...
                bot.sendMessage(chat_id, "Selamat datang di LAPOR NDAN yang merupakan salah satu tool telegram grup NETAR & IS OP Witel SMG " + 
                                        ...
                                        "Jika masih butuh info lebih lanjut harap hubungi @najarmu , @Fikry_id atau @ronaldowidianto, Terima Kasih", parse_mode="HTML")
